...
/**
  ...
  * @return 輝度値の平均値
  */
export function toGray(rgba, gray) {
...
/**
  ...
  * @return なし
  */
export function drawGrayImage(gray, x, y, context, imageData) {
...
/**
  ...
  * @return なし
  */
export function drawColorImage(rgba, x, y, context, imageData) {
  imageData.data.set(rgba); // imageDataに結果を設定する
  context.putImageData(imageData, x, y); // 画像の描画
}
...
/**
  ...
  * @return なし
  */
export function drawLine(context, x1, y1, x2, y2, width, color) {
...
/**
  ...
  * @return なし
  */
export function drawCircle(context, x, y, r, width, color) {
  context.beginPath();    // パスを開始する
  context.lineWidth = width;  // 線の太さ
  context.strokeStyle = color;   // 線の色
...
/**
  ...
  *         しない場合 null
  */
export function matchSAD(template, templateWidth, templateHeight, 
...
　*    もしくは Float32Array(width * height)
  * @return なし
  */
var __g = new Uint8Array(9);
export function toEdge(gray, width, height, fx, fy, amp, edge) {
  var g = __g;
  var total = 0, n = 0;
 ...
 * gray配列の画像にガウスフィルタを適用する
  ...
  * @return なし
  */
export function toGauss(gray, width, height, result) {
  // コンボリューション処理する対象ピクセルの輝度値
  var g = __g;
  var total = 0;
...
/**
  ...
  * @return 一致した場合 true
  */
export function match(template, width, height, target) {
...
/**
  ...
  * @return なし
  */
var __table = new Uint8Array(256); // 変換表 00000にする
export function toBinary(gray, thr, binary) {
  for (var i = 0; i < ~~(thr + 0.5); i++) 
    __table[i] = 0;
  for (var i = ~~(thr + 0.5); i < 256; i++)
    __table[i] = 255; // 表を初期化する
 
  var length = gray.length;
  for (var i = 0; i < length; i++) {
    binary[i] = __table[gray[i]]; // 表引きをする
  }
}
...
/**
  ...
  * @return なし
  */
export function toReverse(gray, reverse) {
  var length = gray.length;
  for (var i = 0; i < length; i++) {
...
/**
  ...
  * @return なし
  */
export function houghLine(binary, width, height, MAX_THETA, MAX_RHO, thetaRho) {
  var max_rho = width * width + height * height; // 対角線の長さの2乗
  // 画像ファイルのサイズのチェック
  if (max_rho > MAX_RHO * MAX_RHO) {
    console.log("画像ファイルが大きすぎる: 幅 " + width + "高さ" + height);
  }
...
/**
  ...
  * @return 見つかった直線の本数
  */
export function invHoughLine(thetaRho, MAX_THETA, MAX_RHO, MAX_LINES, _theta, _rho) {
  if (MAX_LINES <= 0) {
...
/**
  ...
  *         null 交わらない場合
  */
export function houghCross(theta1, rho1, theta2, rho2, MAX_THETA) {
...
/**
  ...
  * @return なし
  */
export function labeling(binary, width, height, labels, table, areas) {
  // ラベル付けする
  ...
  for (var i = 0; i < 30; i++) // console.log("table", i, table[i]);
  for (var y = 1; y < height - 1; y++) {
    for (var x = 1; x < width - 1; x++) {
      var i = x + y * width; 	// 処理しているピクセル(x, y)
...
　* labelと一致するか、調査済み(値が1)場合は、
  ...
  */
export function getLabel(labels, label, x, y, width, traced) {
  var i = x + y * width;
...
/**
  ...
  */
export function trace(labels, width, height, label, points, traced) {
  ...
  for (var y = 1; y < height - 1; y++) { // ラベリング画像が対象なので1で始まる
    for (var x = 1; x < width - 1; x++) {
      ...
      if (getLabel(labels, label, x - 1, y + 1, width, height, traced) <= 0)   // 左下
        if (getLabel(labels, label, x, y + 1, width, height, traced) <=0)      // 下
          if (getLabel(labels, label, x + 1, y + 1, width, height, traced) <= 0) // 右下
            if (getLabel(labels, label, x + 1, y, width, height, traced) <= 0)   // 右
              return 0; // 孤立点  
      ...
      while (loop) {  // (4) 開始点と追跡点が同じ座標なるまで追跡する
        ...
        switch (dir) {
          case 0: // 左上を調べる
            var l = getLabel(labels, label, x - 1, y - 1, width, traced);
            if (l > 0) {
          ...
          case 1: // 左を調べる
            var l = getLabel(labels, label, x - 1, y, width, traced);
            if (l > 0) {
              if (!loop && l == 1) continue;  // (5) 追跡終了
              else loop = true;
              x = x - 1; length++;
          ...
          case 3: // 下を調べる
            var l = getLabel(labels, label, x, y + 1, width, traced);
            if (l > 0) {
              if (!loop && l == 1) continue; // (5) 追跡終了
              else loop = true;
              y = y + 1; length++;
...
/**
  ...
  * @param marker 元のマーカーのImageDataオブジェクト画像(正方形)
  * @param size 一辺の大きさ
  ...
  * @return なし
  */
export function getRotatedMarkers(marker, size, rad90, context, markers) {
  ...
  for (var i = 0; i < 4; i++) {
    ...
    var thr = toGray(imageData.data, markers[i]); // グレースケール化
    toBinary(markers[i], thr, markers[i]); // 2値化
    context.restore();
  }
}
...
/**
  ...
  * @return trueかfalse
  */
export function checkRectangle(corners) {
...
/**
  ...
  * @return 距離
  */
export function distancePointAndLine(points, indexP, index0, index1) {
...
/**
  ...
  * @param corners 4隅を表す座標 [x, y, ...] 反時計回り
  * @param width, height 透視変換前の長方形の幅と高さ
  * @param matirx 透視変換行列を返す3x3行列 matrix = new Float32Array(9);
  * @return なし
  */
export function getHomography(corners, width, height, matrix) {
...
/**
  ...
  */
export function multMatrix3Vec3(matrix, x, y, z, xyz) {
...
/**
  ...
  */
var xyz = new Float32Array(3); // 透視変換後の座標
export function transform(binary, width, height, matrix, tMarker, twidth, theight) {
  for (var y = 0; y < theight; y++) { // 正面を向ける
    for (var x = 0; x < twidth; x++) {
      multMatrix3Vec3(matrix, x, y, 1, xyz); // 座標変換する
      var o_i = ~~(xyz[0] / xyz[2] + 0.5) + (~~(xyz[1] / xyz[2] + 0.5)) * width;
      var t_i = x + y * twidth;
...
/**
  ...
  * @return なし
  */
var __c = new Array(4); // 4隅の点を表す添え字
export function find4Corners(points, corners) {
  __c[0] = findFarthestPoint(points, 0, points.length, 0);
  __c[2] = findFarthestPoint(points, 0, points.length, __c[0]);
  __c[1] = findFarthestPointFrom2(points, _c[0],__c[2],__c[0],__c[2]);
  __c[3] = findFarthestPointFrom2(points,__c[2],__c[0],__c[0],__c[2]);

  var n = 0;
  for (var i = 0; i < 4; i++) {
    corners[n] = points[__c[i]], corners[n + 1] = points[__c[i] + 1];
    n += 2;
  }
}
...
/**
  ...
  * @return 最も遠い点の添え字
  */
export function findFarthestPoint(points, start, end, index) {
...
/**
  ...
  * @param points 輪郭線を構成するピクセルの座標[x, y, ...]
  * @param start, end 点を探す範囲
  * @param index0, index1 上記の2点を示す配列の添え字
  * @return 最も遠い点の添え字
  */
export function findFarthestPointFrom2(points, start, end, index0, index1) {
  var far_index = 0;  // 最も遠い点
  var max_length = 0; // 最も遠い距離
  if (start < end) {
    for (var i = start; i < end && points[i] !== 0; i += 2) {
      var length = distancePointAndLine(points, i, index0, index1);
      if (max_length > length) continue;
      max_length = length, far_index = i;
    }
    return far_index;
